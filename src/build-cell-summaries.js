...
const CSV_FILES = 'input-data/image-store/vccf-data-cell-nodes/published/*/*-nodes.csv';
...
for (const nodesFile of globSync(CSV_FILES).sort()) {
  ...
  if (existsSync(datasetFile)) {
    ...
    datasetCounts[dataset['@id']] = totalCells;
...
const datasets = results.map((summary) => ({
  ...
  dataset_cell_count: datasetCounts[summary['cell_source']],
}));
